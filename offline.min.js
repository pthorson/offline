...
(function() {
    ...
    extendNative = function(to, from) {
        ...
        for (key in from.prototype) try {
            val = from.prototype[key], results.push(null == to[key] && "function" != typeof val ? to[key] = val : void 0);
        } catch (_error) {
            e = _error;
        }
        return results;
    ...
    defaultOptions = {
        ...
        cur = cur[part], "object" == typeof cur && !typeIsArray(cur)); i = ++j) ;
        ...
        if (null != handlers[event]) {
            if (handler) {
                ...
                ctx = ref[0], _handler = ref[1], results.push(_handler === handler ? handlers[event].splice(i, 1) : i++);
                return results;
            }
        ...
        return checkStatus = function() {
            ...
            return 4 === xhr.readyState ? checkStatus() : 0 === xhr.readyState && onDown(), 
    ...
    var clear, flush, held, holdRequest, makeRequest, waitingOnConfirm, indexOf = [].indexOf || function(item) {
    ...
    held = [], waitingOnConfirm = !1, holdRequest = function(req) {
        ...
        return Offline.getOption("requests") !== !1 ? (Offline.on("confirmed-up", function() {
            ...
            return xhr = request.xhr, async = request.async, url = request.url, xhr.offline === !1 || (requestFilters = Offline.getOption("requestFilters"), 
            extension = url.split(".").pop().split("?")[0], indexOf.call(requestFilters, extension) >= 0 || (hold = function() {
                return holdRequest(request);
            }, _send = xhr.send, xhr.send = function(body) {
                return request.body = body, _send.apply(xhr, arguments);
            }, !async)) ? void 0 : null === xhr.onprogress ? (xhr.addEventListener("error", hold, !1), 
