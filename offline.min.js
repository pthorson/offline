...
(function() {
    ...
    held = [], waitingOnConfirm = !1, holdRequest = function(req) {
        ...
        return Offline.getOption("requests") !== !1 ? (Offline.on("confirmed-up", function() {
            ...
            if (xhr = request.xhr, async = request.async, url = request.url, xhr.offline !== !1) {
                if (requestFilters = Offline.getOption("requestFilters"), extension = url.split(".").pop().split("?")[0], 
                indexOf.call(requestFilters, extension) >= 0) return void console.log("block ", url);
                if (console.log("authorize", url), hold = function() {
